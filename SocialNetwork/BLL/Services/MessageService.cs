    ...
    public class MessageService
    {
        ...
        public IEnumerable<Message> GetIncomingMessagesByUserId(int recipientId)
        {
            ...
            messageRepository.FindByRecipientId(recipientId).ToList().ForEach(m =>
            {
                ...
                var recipiendUserEntity = userRepository.FindById(m.recipient_id);

                messages.Add(new Message(m.id, m.content, senderUserEntity.email, recipiendUserEntity.email));
            });
            return messages;
        }
